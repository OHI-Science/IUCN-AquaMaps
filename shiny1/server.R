...
shinyServer(function(input,output){
  ...
  map_compare <- reactive({
    ...
    if(input$top == 'iucn') {
      plot(r_am_spp, col = am_pres_cols, main = input$species, useRaster = FALSE,  legend = (input$am_form == 'prob'), axes = FALSE, alpha = input$am_trans/100)
      plot(r_iucn_spp, col = iucn_cols, useRaster = FALSE,  legend = FALSE, axes = FALSE, alpha = input$iucn_trans/100, add = TRUE)   
    } else {
      plot(r_iucn_spp, col = iucn_cols,  main = input$species, useRaster = FALSE, legend = (input$am_form == 'prob'), axes = FALSE, alpha = input$iucn_trans/100)         
      plot(r_am_spp, col = am_pres_cols, useRaster = FALSE,  legend = FALSE, axes = FALSE, alpha = input$am_trans/100, add = TRUE)
    }
    map('world', col = 'gray95', fill = T, border = 'gray80', add = TRUE)
  })
