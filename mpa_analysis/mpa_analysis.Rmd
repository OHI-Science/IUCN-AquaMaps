...
### import OHI regions polygons, to determine spatially-marine areas
  poly_marine_file <- file.path(dir_anx, 'explore_am_v_iucn/mpa_rasters/poly_marine1.shp')
  if(!file.exists(poly_marine_file)) {
    ...
    poly_list <- list()
    poly_rows <- nrow(poly_marine)
    n_entities <- 20
    for (i in 1:ceiling(poly_rows/n_entities)) { # i = 1
      min_row <- (i - 1) * n_entities + 1
      max_row <- min(i * n_entities, poly_rows)
      poly_tmp <- poly_marine[min_row:max_row, ]
      message(sprintf('%s. Entities [%s:%s]', i, min_row, max_row))
      print(system.time({
        poly_tmp2 <- rgeos::gUnaryUnion(poly_tmp)
      })[3])
      poly_list[i] <- poly_tmp2
    }

    poly_mar_frankenstein <- spRbind(poly_list)
    
    # poly_mar_union <- rgeos::gUnaryUnion(poly_marine)
    
    # writePolyShape(poly_marine, poly_marine_file)
    writeOGR(obj    = poly_mar_frankenstein,
    ...
    poly_marine <- readShapePoly(poly_marine_file, 
                               proj4string = crs_wgs84)
    }
  
  plot(poly_marine, col = 'blue')
  
  # rast_marine <- rasterize(poly_marine, rast_base, value = sp_mar_flag) 
  ### excruciatingly slow (interminable?)... consider gdal_rasterize?
