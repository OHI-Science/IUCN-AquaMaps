...
```{r global map matched species, echo = FALSE, warning = FALSE, message = FALSE, eval = FALSE}

library(rasterVis)

cat('First off: map-paired species.  Map 1: AM dataset; map 2: IUCN dataset; maps 3 & 4: log-transformed AM & IUCN\n')
...
iucn_pair_rast_file <- file.path(dir_data, 'rasters/iucn_pair_rast.tif')
am_pair_rast_file   <- file.path(dir_data, 'rasters/am_pair_rast.tif')
...
cat('Full AM dataset, species richness\n')
...
#                           main = 'Entire Aquamaps Dataset',
                           par.settings = myTheme)

# all_am_ln_map <- log(all_am_map + 1)
# all_am_ln_map <- all_am_ln_map/maxValue(all_am_ln_map)
# am_levelplot1 <- levelplot(all_am_ln_map, par.settings = myTheme, main = 'Entire Aquamaps Dataset: log-transformed relative species richness', 
...
cat('Full IUCN dataset, species richness\n')
