...
### here are mean lines:
# scatter_arearatio_vs_overlap <- scatter_arearatio_vs_overlap +
#   geom_vline(x = area_align_mean,
#              color = 'grey50', linetype = 'dashed') +
#   geom_hline(y = dist_align_mean,
#              color = 'grey50', linetype = 'dashed')
...
scatter_arearatio_vs_overlap <- scatter_arearatio_vs_overlap +
  labs(x = bquote('Area alignment '*alpha['ext']*' (%)'), 
...
``` {r plot_spp_iucn_gaps_tenplus, eval = TRUE, echo = FALSE}

iucn_gap10     <- read_csv(file.path(dir_data, 'iucn_gaps_tenplus.csv'))
iucn_gap10_spp <- unique(iucn_gap10[iucn_gap10$sciname %in% spp_list$sciname, ])
...
for (spp in iucn_gapmaps$sciname) { # i = 1
  spp_plot <- plot_rangemap(spp)
  spp_solo <- spp_list %>% filter(sciname == spp)
  spp_plot <- spp_plot +
    labs(title = sprintf('%s: %s', spp, spp_solo$spp_group_text))
...
``` {r get_iucn_citation_info, echo = FALSE, warning = FALSE, eval = FALSE}
