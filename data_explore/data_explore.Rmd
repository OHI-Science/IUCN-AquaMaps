...
spp_class_all <- am_maps_class %>%
  ...
  mutate(am_class = ifelse(am_class %in% c('null', 'not assigned'), NA, am_class),
         class    = if_else(is.na(am_class), iucn_class, am_class))

### Fix some missing class values
# spp_class_all1 %>% filter(is.na(class))
#       am_sid                   sciname am_class iucn_sid sciname_iucn iucn_class class
# 1  Fis-22442 Metapenaeopsis tarawensis     <NA>       NA         <NA>       <NA>  <NA> # Malacostraca
# 2 Fis-170350          Oenopota eriopis     <NA>       NA         <NA>       <NA>  <NA> # Gastropoda
# 3 Fis-170316          Yarica hyalosoma     <NA>       NA         <NA>       <NA>  <NA> # Actinopterygii
spp_class_all <- spp_class_all %>%
  mutate(class = case_when(
    is.na(.$class) & .$am_sid == 'Fis-22442' ~ 'Malacostraca',
    is.na(.$class) & .$am_sid == 'Fis-170350' ~ 'Gastropoda',
    is.na(.$class) & .$am_sid == 'Fis-170316' ~ 'Actinopterygii',
    TRUE ~ .$class))
...
taxa_all <- taxa_am_all %>%
  ...
  mutate(small_class = (is.na(n_iucn_class_all) & p_am_class_all < 0.5))
