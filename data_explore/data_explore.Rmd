...
``` {r plot_bar_charts_by_taxa_old, echo = FALSE, message = FALSE, warning = FALSE, eval = FALSE}
...
# 
# bar_taxa <- ggplot(taxa_df_long, aes(x = class, y = percent, fill = dataset)) +
#   ggtheme_plot + 
...
# 
# print(bar_taxa)
# ggsave(filename = file.path(dir_fig, 'fig1a_bar_by_taxa_all.png'), 
...
taxa_df1 <- taxa_df %>%
  ...
  mutate(AquaMaps = ifelse(is.na(AquaMaps), 0, AquaMaps),
         ...
         n_total  = AquaMaps + IUCN + Both,
         IUCN_end = 100 * (IUCN + Both)/n_total,
         IUCN_start = 0,
         AquaMaps_end   = 100,
         AquaMaps_start = 100 * (n_total - AquaMaps - Both)/n_total,
         Both_end = AquaMaps_start,
         Both_start = IUCN_end) %>%
...
taxa_all_df <- taxa_df1 %>%
  ...
  summarize(class = 'Total',
            AquaMaps_total = sum(AquaMaps) + sum(Both),
            IUCN_total = sum(IUCN) + sum(Both),
            n_total  = sum(n_total),
...
bar_taxa <- ggplot(taxa_df2, aes(x = class, color = dataset)) +
  ...
  scale_size_manual(values = c('IUCN' = .25, 'AquaMaps' = .25, 'Both' = 2.5)) +
  scale_color_manual(values = c('IUCN' = 'black', 'AquaMaps' = 'grey60', 'Both' = 'black')) +
...
ggsave(file.path(dir_fig, 'fig2a_scatter_quadmap.png'),
...
ggsave(plot = barchart_spp_gp_quads,
       filename = file.path(dir_fig, 'fig2b_barchart_spp_gp_quads.png'),
       width = 17.8, height = 7, units = 'cm', dpi = 600)
