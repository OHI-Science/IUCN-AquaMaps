...
area_align_med <- median(spp_list$area_ratio, na.rm = TRUE)
dist_align_med <- median(spp_list$sm_perc, na.rm = TRUE)
...
scatter_arearatio_vs_overlap <- ggplot(spp_list %>% 
  ...
  ### color the quadrant backgrounds:
    annotate("rect", xmin = area_align_med, xmax = 100, 
                     ymin = dist_align_med, ymax = 100, 
             alpha = .3, 
#             fill= "green4")  + 
             fill= "#4dac26")  + 
    annotate("rect", xmax = area_align_med, xmin =   0, 
                     ymin = dist_align_med, ymax = 100, 
             alpha = .3, 
#             fill= "greenyellow") + 
             fill= "#b8e186") + 
    annotate("rect", xmin = area_align_med, xmax = 100, 
                     ymax = dist_align_med, ymin =   0, 
             alpha = .3, 
#             fill= "orange") + 
             fill= "#f1b6da") + 
    annotate("rect", xmax = area_align_med, xmin =   0, 
...
scatter_arearatio_vs_overlap <- scatter_arearatio_vs_overlap +
  annotate(geom = 'text',
           x = area_align_med, y = 5,
           hjust = 0, vjust = 0,
           color = 'grey30', 
           size = 2,
           fontface = 'bold.italic', angle = 90,
           label = sprintf('Median = %s%%', round(area_align_med, 1))) +
  annotate(geom = 'text',
           x = 5, y = dist_align_med,
           hjust = 0, vjust = 0,
           color = 'grey30', 
           size = 2,
           fontface = 'bold.italic', angle = 0,
           label = sprintf('Median = %s%%', round(dist_align_med, 1)))
  
### here are median lines:
# scatter_arearatio_vs_overlap <- scatter_arearatio_vs_overlap +
#   geom_vline(x = area_align_med,
#              color = 'grey50', linetype = 'dashed') +
#   geom_hline(y = dist_align_med,
#              color = 'grey50', linetype = 'dashed')
...
taxa_df_long2 <- taxa_df %>%
  ...
  mutate(n_total = sum(n_spp, na.rm = TRUE),
         ...
         dataset = factor(dataset, levels = c('AquaMaps', 'Both', 'IUCN')))
...
#  scale_fill_manual(values = c('AquaMaps' = 'red', 'Both' = 'mediumorchid', 'IUCN' = 'blue')) +
  scale_fill_manual(values = c('AquaMaps' = '#1b9e77', 'Both' = '#d95f02', 'IUCN' = '#7570b3')) +
...
``` {r get_iucn_citation_info, echo = FALSE}
...
dir_iucn_shp <- file.path(dir_N, 'git-annex/globalprep/_raw_data/iucn_spp/iucn_shp')
...
for (spp_gp in groups_list$shp_fn) { # spp_gp <- groups_list$shp_fn[3]
  spp_dbf <- foreign::read.dbf(file.path(dir_iucn_shp, sprintf('%s.dbf', spp_gp)))
  message('file read successfully... ')
  cites <- spp_dbf %>%
    select(id_no, compiler, year, citation) %>%
    unique() %>%
    mutate(spp_group = spp_gp)

  iucn_shp_info <- bind_rows(iucn_shp_info, cites)
  message('binding to list...\n')
}

### oldest records go back to late 70s for terrestrial mammals; then skip to 2004 for a marine turtle map.
...
``` {r, child = 'data_explore_am_thresh.Rmd', echo = FALSE, message = FALSE, eval = FALSE}
```
