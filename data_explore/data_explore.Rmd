...
ggsave(file.path(dir_fig, 'AreaRatiovsPercOverlap.png'), width = 6, height = 3.5)
...
spp_list <- spp_list %>% mutate(data_poor = ifelse(occurcells < 10, '< 10', '>= 10')
### mongo plot time
scatter_arearatio_vs_overlap <- ggplot(spp_list %>% 
                                         transform(popn_category = reorder(popn_category, category_score)),
...
ggsave(file.path(dir_fig, 'AreaRatiovsPercOverlap-data_poor.png'), width = 6, height = 3.5)
...
barchart_spp_gp_quads <- ggplot(spp_gp_quadrants %>% 
  ...
  geom_text(aes(label = sprintf('n = %s', n_spp), y = .05), hjust = 0, size = 2, color = 'grey20') +
  coord_flip() +
...
write.csv(spp_gp_quadrants, file.path(dir_data, 'spp_gp_quads.csv'), row.names = FALSE)
...
iucn_marine_file <- file.path(dir_data, 'spp_iucn_marine.csv')
...
bar_taxa2 <- ggplot(taxa_df_long2, aes(x = class, y = percent, fill = dataset)) +
  ...
  geom_text(aes(label = sprintf('n = %s', n_total), y = 3), 
            size = 3, angle = 90, hjust = 0, color = 'grey30') +
  theme(axis.text.x = element_text(angle = 75, hjust = 1)) + 
  scale_y_continuous(expand = c(0, 0), limits = c(0, 100)) + 
  labs(x = 'Class', y = 'Percent of mapped species',
       title = 'Species maps by taxonomic class')
print(bar_taxa2)
ggsave(filename = file.path(dir_fig, 'bar_by_taxa_all.png'), height = 3, width = 6)
...
bar_taxa3 <- ggplot(taxa_df_long3, aes(x = class, y = percent, fill = dataset)) +
  ...
  theme(axis.text.x = element_text(angle = 75, hjust = 1)) + 
  scale_y_continuous(expand = c(0, 0), limits = c(0, 100)) +
  labs(x = 'Class', y = 'Percent of assessed mapped species',
       title = 'Species maps by taxonomic class, Red List-assessed only')
print(bar_taxa3)
ggsave(filename = file.path(dir_fig, 'bar_by_taxa_assessed.png'), height = 3, width = 6)
