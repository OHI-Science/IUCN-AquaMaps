...
iucn_maps_class_all <- spp_iucn_maps %>% 
  select(-iucn_subpop, -spp_group) %>%
  distinct() %>% ### first, get rid of dupes due to subpopulations
  mutate(class    = str_to_title(class),
         am_match = sciname %in% spp_am$sciname)

### next, identify spp scinames with matches in AM; if an iucn_sid has a match,
### keep it; if there are multiple matches, keep the one with the fewest
### instances of sciname (to keep more specific names...?)
iucn_maps_class <- iucn_maps_class_all %>% 
  group_by(sciname) %>%
  mutate(sciname_count = n()) %>% ### for each sciname, figure out how many times it appears
  group_by(iucn_sid) %>%
  arrange(desc(am_match), sciname_count) %>% ### for each iucn_spp, put the AM matches on top; then...
  filter(sciname == first(sciname)) %>% ### just keep the first instance of each iucn_spp
  ungroup()
...
taxa_df2$n_label[duplicated(taxa_df2$class)] <- NA
...
quad_names <- data.frame('quad' = c('q1', 'q2', 'q3', 'q4'),
                         'quad_name' = factor(c('well-aligned', 'dist-aligned', 
                                                'area-aligned', 'poorly aligned'),
                                             ordered = TRUE))
