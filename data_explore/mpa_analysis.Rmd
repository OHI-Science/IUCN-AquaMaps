...
* to avoid double-counting for overlapping polygons - rasterize WDPA to fine scale first
    ...
    * WDPA_MPA layer: value = IUCN category (1-6, with NA = 7?) combined with marine flag and marine overlap flag?
...
rast_base      <- raster(ext = extent(rast_loiczid), resolution = 0.01)
rast_area_base <- raster::area(rast_base) ### takes a while...
...
### filter out all polygons that are land or landlocked, and flag the rest as spatially-marine = 1
poly_marine <- poly_rgn[!str_detect(poly_rgn@data$rgn_typ, 'land'), ]
poly_marine@data <- poly_marine@data %>%
  mutate(sp_mar_flag = 1)

# writePolyShape(poly_marine, file.path(dir_anx, 'explore_am_v_iucn/tmp_rasters/poly_sp_mar'))

# rast_sp_mar <- rasterize(poly_marine, rast_base, value = sp_mar_flag) ### pretty slow... consider gdal_rasterize?

# library(gdalUtils)
# rast_sp_mar <- gdal_rasterize(
#     src_datasource = file.path(dir_anx, 'explore_am_v_iucn/tmp_rasters/poly_sp_mar.shp')),
#     dst_filename = file.path(dir_anx, 'explore_am_v_iucn/tmp_rasters/poly_sp_mar.tif'), 
...
poly_wdpa@data <- poly_wdpa@data %>%
  ...
  mutate(prot_code = prot_code + MARINE)
