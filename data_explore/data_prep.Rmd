...
spp_iucn <- read_csv(file.path(dir_anx, scenario, 'int/spp_iucn_cleaned.csv'))
spp_iucn <- spp_iucn %>%
...
spp_iucn_maps <- read_csv(file.path(dir_anx, scenario, 'int/spp_iucn_maps_all.csv')) %>%
...
spp_am <- read_csv(file.path(dir_anx, scenario, 'int/spp_am_cleaned.csv'))
write_csv(spp_am, file.path(dir_data, 'spp_am.csv'))

### pull raw aquamaps species list from git-annex and bring to local repo
spp_am_raw <- read_csv(file.path(dir_data_remote, 'aquamaps/v2015/csv/speciesoccursum.csv'))
...
spp_all_remote <- read_csv(file.path(dir_anx, scenario, 'int/spp_all.csv'))
...
spp_iucn_maps_new <- read_csv(file.path(dir_data, 'spp_iucn_maps.csv')) %>%
...
### - aliases, etc.
spp_am_new <- read_csv(file.path(dir_data, 'speciesoccursum.csv')) %>%
...
### This file is generated in globalprep/SPP_ICO:
iucn_spp_cells_all <- read_csv(file.path(dir_data_remote, 'iucn_spp/iucn_cells_v2016.csv'),
                            col_types = 'cddddc')
...
iucn_spp_cells <- iucn_spp_cells_all %>%
  ...
  filter(presence != 5)
### length(unique(iucn_spp_cells$iucn_sid)) results in 2279 species found...
### some species with no cells reported.  2279 listed in the am v iucn file.
...
am_spp_cells_all <- fread(file.path(dir_data_remote, 'aquamaps/v2015/csv/hcaf_sp_native_trunc.csv'), 
                         showProgress = TRUE)
am_spp_cells <- am_spp_cells_all %>%
  rename(am_sid = speciesid, prob = probability) %>%
  filter(am_sid %in% spp_am_v_iucn$am_sid)
### length(unique(am_spp_cells$am_sid)) results in 2314 species found - great!
