...
system.time({
  x <- occ_search(scientificName = spp_all$sciname, fields = 'minimal', limit = 1)
...
z <- data.frame(sciname = names(y), obs = y)
...
z <- z %>% 
  ...
  left_join(read_csv(file.path(dir_data_remote, 'aquamaps/v2015/csv/speciesoccursum.csv')) %>%
                       select(am_sid = speciesid, occurcells),
                     by = 'am_sid') %>%
  mutate(ln_obs        = log(obs + 1),
         ln_occurcells = log(occurcells + 1))

plot(ln_obs ~ ln_occurcells, data = z)
plot(obs ~ occurcells, data = z)
print(summary(lm(obs ~ occurcells, data = z)))

write_csv(z, file.path(dir_anx, 'explore_am_v_iucn/occurrence_df.csv'))
