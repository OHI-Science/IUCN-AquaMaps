...
dir_aquamaps <- file.path(dir_N, 'git-annex/globalprep/_raw_data/aquamaps/v2015')
...
hist_area_vs_threshold <- ggplot(spp_df3, aes(prop, fill = iucn_code)) + 
  ...
  labs(x = 'Proportion of range included when selecting a threshold of 40%', 
       y = sprintf('Proportion of all aquamaps species (n = %s)', nrow(spp)),
       fill = 'IUCN code')
...
#  geom_point(position = position_jitter(w = .1), aes(color = scenario), alpha = .5) +
  ...
  geom_ribbon(aes(ymax = quant95, ymin = quant05), alpha = .5, color = NA, fill = '#b8e186') +
  geom_ribbon(aes(ymax = quant75, ymin = quant25), alpha = .5, color = NA, fill = '#4dac26') +
