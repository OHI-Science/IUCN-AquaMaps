...
iucn_spp_cells <- read_csv(file.path(dir_anx, 'explore_am_v_iucn/iucn_spp_cells_both_2015.csv'), 
...
if(!file.exists(data_file)) {
  ...
  spp_list_base <- read.csv('shiny/data/spp_am_v_iucn_2015.csv', stringsAsFactors = FALSE)
...
```{r global_dist_map, echo = FALSE, warning = FALSE, message = FALSE}
...
all <- all_am %>%
  left_join(hcaf, by = 'CsquareCode')

all_am_map <- subs(loiczid_raster, all[ , c('loiczid', 'all_count')], 
                   by = 'loiczid', 
                   which = 'all_count', 
                   subsWithNA = TRUE)
