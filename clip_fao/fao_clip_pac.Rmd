...
rast_spp_e <- crop(rast_spp_e, extent(c(85, 220, -60, 60)))
...
rast_spp_map_e <- 
  tm_shape(rast_spp_e) +
    tm_raster(palette = 'Reds',
              ...
              alpha = 1) +
  tm_shape(World) +
    tm_polygons(border.col = 'grey50', col = 'grey55') + 
  tm_shape(fao_polys) +
...
rast_list <- vector('list', length = nrow(spp_examples_e))
...
for (i in 1:nrow(spp_examples_e)) {  # i <- 1
  ...
  r1 <- r_am_spp %>%
    crop(extent(c(-180, 0, -50, 50)))
  extent(r1) <- c(180, 360, -50, 50)
  
  r2 <- r_am_spp %>%
    crop(extent(c(0, 180, -50, 50)))
  
  r3 <- merge(r2, r1)
  
  new_ext <- extent(c(50, 240, -50, 50))
  r_am_crop <- crop(r3, new_ext)
  
  rast_list[i] <- r_am_crop
  names(rast_list)[i] <- spp
  
  spp_plot <- assemble_map(r_am_crop, spp$sciname)
}
...
mapstack_e <- crop(mapstack_e, extent(c(85, 220, -50, 50)))


stack_plot_e <- assemble_map(mapstack_e, 'n_spp') +
  tm_shape(fao_polys) +
  tm_borders()
print(stack_plot_e)
...
rast_list <- vector('list', length = nrow(spp_examples_w))

for (i in 1:nrow(spp_examples_w)) {  # i <- 1
  
  spp <- spp_examples_w[i, ]
  map_cells <- am_spp_cells %>%
    filter(am_sid == spp$am_sid) %>%
    mutate(am_pres = 1)
...
mapstack_w <- stack(rast_list) %>%
  calc(sum, na.rm = TRUE)
plot(mapstack_w)
values(mapstack_w)[values(mapstack_w) == 0] <- NA
mapstack_w <- crop(mapstack_w, extent(c(85, 220, -50, 50)))

stack_plot_w <- assemble_map(mapstack_w, 'n_spp') +
  tm_shape(fao_polys) +
  tm_borders()
