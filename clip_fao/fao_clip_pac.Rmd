...
spp_plotted <- read_csv('data/spp_all.csv') %>%
  ...
  left_join(read_csv('data/speciesoccursum.csv') %>%
              select(am_sid = speciesid, occurcells, fbname),
            by = 'am_sid')
...
``` {r plot_example_spp, echo = FALSE, warning = FALSE, message = FALSE, eval = TRUE}
...
for(i in 1:length(spp_ex)) {
  ...
  spp_occur <- read_csv(paste0('clip_fao/obs_for_', tolower(spp) %>% 
    ...
    select(id, long = decimalLongitude, lat = decimalLatitude) %>%
    mutate(long = ifelse(long < 0, long + 360, long))
  spp_occur_pts <- SpatialPoints(spp_occur[ , 2:3])
  ...
  values(rast_spp)[values(rast_spp_w) == 0] <- NA
  # values(rast_spp_w)[is.na(values(rast_spp_w))] <- 0
