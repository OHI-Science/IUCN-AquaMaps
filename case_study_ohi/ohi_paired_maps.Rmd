...
# load IUCN cells, filter to just paired maps
spp_cells_iucn

# load AM cells, filter to just paired maps; include column of prob of occur
...
clipped_quads <- ggplot(iucn_coral_fixed %>%
  ...
  # geom_point(color = 'grey80', size = 2, alpha = 1) + ### blank behind points
  geom_point(aes(color = method), size = 2, alpha = .7) +
  scale_color_manual(values = c('original' = '#4d4dac', '200 m clip' = '#00cc00')) +
