...
if(!file.exists(am_all_rast_file)) {
  ...
  writeRaster(all_am_map, filename = am_all_rast_file, overwrite = TRUE)
...
all_iucn_rast_file   <- 'rasters/am_all_rast.tif'

if(!file.exists(iucn_all_rast_file)) {
  ...
  all_iucn_map   <- raster(iucn_all_rast_file)
}
...
am_map_max <- max(all_am_map, na.rm = TRUE)
all_am_pct_map <- all_am_map/am_map_max

iucn_map_max <- max(all_iucn_map, na.rm = TRUE)
