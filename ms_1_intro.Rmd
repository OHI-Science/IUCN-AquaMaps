...
Here we focus not on how the two datasets should ideally be used; instead, in recognizing that each provides value for many aspects of conservation research, we highlight hidden assumptions and sources of error within each dataset, and explore the implications of choosing one over the other.  _You science with the data you have, not with the data you wish you had.  Often, the ideal type of spatial data is not always available, and non-ideal data is better than no data at all.  In such a case, it is crucial to acknowledge, and where possible address, the resulting tradeoffs._
...
___maybe here, based on EOO/AOO defs, we expect AM to generally fall within, and describe a smaller range than, IUCN; where the two disagree, that's where it gets interesting___
