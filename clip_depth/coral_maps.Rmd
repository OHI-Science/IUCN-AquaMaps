...
rast_coral_comp <- subs(x = loiczid_raster, y = coral_comp %>% select(loiczid, deep), 
...
map_coral_comp <- 
  tm_shape(rast_coral_comp) +
    tm_raster(breaks = c(0, 200, Inf),
              palette = c('lightblue', 'royalblue4'),
              colorNA = NULL,
              title = 'depth (meters)',
              alpha = 1) +
  tm_shape(World) +
...
save_tmap(map_coral_comp,
          filename = file.path(dir_fig, 'fig3a_coral_map.png'), 
          width = 9, height = 4.2, units = 'cm', dpi = 600)
...
write_csv(iucn_coral_quads, file.path(dir_git, 'clip_depth', 'iucn_coral_quads.csv'))

break_nums <- seq(0, 100, 20)

iucn_coral_quad_sum <- read_csv(file.path(dir_git, 'clip_depth', 'iucn_coral_quads.csv')) %>%
