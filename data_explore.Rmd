...
# Data files
#iucn spp cells WITHOUT extinct locations of IUCN species
iucn_spp_cells <- read_csv(file.path(dir_anx, 'explore_am_v_iucn/iucn_spp_cells_both.csv'), col_types = 'cdddd', progress = TRUE)%>%as.data.frame()%>%filter(presence!=5)
...
ggsave('figures/sp_groups_barchart.png',plot=g,width=8, height=4, dpi=100)
print(g)
...
dot = ggplot(df,aes(iucn_area,am_area,color=iucn_category))+ geom_point() + geom_abline()+
      labs(x='Area (km2) from IUCN dataset',y='Area (km2) from AquaMaps dataset',title='Comparing total area of species range maps')
ggsave('figures/CompareTotalArea_AM-IUCN_scatter.png',plot=dot,width=8, height=4, dpi=100)
print(dot)
```
...
all_status <- ggplot(df,aes(iucn_log,am_log,color=iucn_category)) + 
...
ggsave('figures/CompareTotalAreaLOG_AM-IUCN_scatter.png',plot=all_status,width=8,height=4,dpi=100)
print(all_status)
```
...
groups <- ggplot(df, aes(iucn_log, am_log,color=iucn_category)) + geom_point() +facet_wrap(~spp_group) + 
...
ggsave('figures/CompareTotalAreaLOG_AM-IUCN_spgroups_scatter.png',plot=groups,width=8,height=6,dpi=100)
print(groups)
```
***  
...
overlap = df%>%dplyr::filter(perc.type=='perc')
  
all<-ggplot(overlap,aes(percent))+
    geom_histogram(aes(y=..count../1926),binwidth=5,alpha=.8)+
    labs(x='% total range that is shared b/w both datasets', y='Proportion of Total Species')
...
sp <- ggplot(overlap,aes(percent))+geom_histogram(binwidth=5,alpha=.8) +
  facet_wrap(~spp_group) +
  labs(x='Percent of total range that is shared between both datasets', y='Number of Total Species')+ 
...
```{r pct small within large, echo=F, warning= F}
...
sm_plot <- ggplot(d, aes(sm_perc, fill = sm_range)) + 
       geom_histogram(aes(y=..count../sum(..count..)),binwidth = 4, alpha = .8) +
        ...
        labs(x = 'Percent of smaller range that falls within the larger range', y = 'Proportion of all species (n=1962)')
...
``` {r pct overlap vs area, echo = FALSE, warning = F, message=F}

reg <- ggplot(spp_list, aes(sm_perc,total_area, color = sm_range)) + 
  geom_point() + 
  geom_abline() + 
  labs(y = 'Total Area (km2)', x = 'Percent overlap smaller range in larger', 
...
p1 <- reg + geom_text(x = 40, y = 300000000, label = lm_eqn(spp_list), parse = TRUE)
...
logA_vs_pct <- ggplot(spp_list, aes(log_total_area, sm_perc, color = sm_range)) + 
  geom_point() + 
  labs(x = 'Log(total Area)', y = 'Percent overlap smaller range in larger', 
...
``` {r examine regressions, echo = FALSE, warning = F}
...
``` {r examine quartiles, echo = FALSE, warning = F}
...
qtiles <- c(quantile(spp_list$total_area, 0.0,na.rm=T),
            quantile(spp_list$total_area, .25,na.rm=T),
            quantile(spp_list$total_area, .50,na.rm=T),
            quantile(spp_list$total_area, .75,na.rm=T),
            quantile(spp_list$total_area, 1.0,na.rm=T))
...
```{r aquamaps probabilities, warning=F,message=F,cache.lazy=FALSE,cache=T,verbose=F,results=F,echo=F,include=F}
...
spp_cells = fread(input=file.path(dir_aquamaps,"tables/ohi_hcaf_species_native.csv"),verbose=F,header=T,
...
spp       = fread(input=file.path(dir_aquamaps,"tables/ohi_speciesoccursum.csv"),verbose=F,header=T)%>%
...
#create dataframe that gives me all species, their total number of cells, total number of cells >=0.4 prob, differnece between the two and threat
df = spp_cells%>%
      group_by(species_id)%>%
        summarize(cells_0.4 = sum(probability>=0.4),
          ...
          select(scientific,species_id,cells_0.4,cells_t,prop,iucn_code)


hist <- ggplot(df,aes(prop,fill=iucn_code))+geom_histogram(aes(y=..count../sum(..count..)),binwidth = 0.05, alpha = .7)+labs(x='Proportion of range area included when selecting a threshold of 0.4',y='Proportion of all aquamaps species (17,348 total)')
