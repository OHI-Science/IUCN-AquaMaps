...
```{r species_map_function, echo = FALSE, warning = FALSE, message = FALSE}
...
scatter_am_iucn_area <- ggplot(spp_df, aes(iucn_area, am_area, color = iucn_category)) + 
  geom_point() + 
  geom_abline(slope = 1, intercept = 0, color = 'red') +
  labs(x = 'Area (km2) from IUCN dataset', 
       y = 'Area (km2) from AquaMaps dataset', 
...
scatter_am_iucn_area_log <- ggplot(spp_df, aes(iucn_log, am_log, color = iucn_category)) + 
  ...
  labs(x = 'IUCN Area (log transformed)', 
       ...
       title = 'Comparing area of range maps\n (all area values are km2 log transformed)', 
       fill = 'IUCN Category')
...
### plot raster of species: IUCN map, AM map, world map, legend
plot(r_iucn_spp, col = 'mediumpurple', 
     useRaster = FALSE, 
     axes = FALSE, 
     box = FALSE, 
     legend = FALSE)

plot(r_am_spp, col = 'orange', 
     main = "Yellow-speckled Chromis", 
     useRaster = FALSE, 
     add = TRUE, 
     axes = FALSE, 
     box = FALSE, 
...
map('world', col = 'gray95', 
    fill = TRUE, 
    border = 'gray80', 
    add = TRUE)

legend("topright", 
       legend = c("AquaMaps", "IUCN"), 
       fill = c('orange', 'mediumpurple'))
...
hist_overlap_pct <- ggplot(overlap, aes(percent)) +
  ...
  labs(x = '% total range that is shared b/w both datasets', 
...
hist_overlap_pct_gp <- ggplot(overlap, aes(percent)) + 
  ...
  facet_wrap(~spp_group) +
  labs(x = 'Percent of total range that is shared between both datasets', 
...
sm_plot <- ggplot(spp_df4, aes(sm_perc, fill = sm_range)) + 
  ...
  labs(x = 'Percent of smaller range that falls within the larger range', 
       y = 'Proportion of all species (n = 1962)')
...
``` {r examine regressions, echo = FALSE, warning = FALSE}
...
``` {r examine quartiles, echo = FALSE, warning = FALSE}
...
hist <- ggplot(spp_df3, aes(prop, fill = iucn_code)) + 
