...
ggplot(d,aes(sm_perc,fill=sm_range))+geom_histogram(binwidth=1,alpha=.7)+
  scale_fill_discrete(name="Dataset with smaller range",labels=c("AquaMaps", "IUCN"))+labs(x='Percent of smalle range that falls within the larger range', y='Number of Species')

reg <- ggplot(spp_list, aes(sm_perc,total_area,color=sm_range)) + geom_point() + geom_abline() + labs(y='Total Area (km2)', x='Percent overlap smaller range in larger', title='Size of species range vs. percent overlap of \nsmaller range with larger')
...
r <- ggplot(l, aes(sm_perc,total_area,color=sm_range)) + geom_point() + labs(y='Total Area (km2)', x='Percent overlap smaller range in larger', title='Size of species range vs. percent overlap of \nsmaller range with larger')
...
#source mapping aquamaps.R to read in all aquamaps raw data. This takes a while so rendering will take a while. 
source('mapping aquamaps.R')
