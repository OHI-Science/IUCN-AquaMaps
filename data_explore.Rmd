...
spp_quads <- c('Kajikia albida', 'Galaxea cryptoramosa', 'Acanthurus nigroris', 'Metanephrops australiensis', 'Conus magnificus')
...
scatter_arearatio_vs_overlap <- ggplot(spp_list, aes(area_ratio, sm_perc, color = sm_range)) + 
  geom_point() + 
  geom_abline() + 
  geom_point(data = spp_list_label, color = 'black', size = 4) + 
...
for (spp in spp_quads) { # spp = 'Kajikia albida'
  map <- get_spp_map(spp)
...
spp_plot <- ggplot(spp_pts, aes(x = x, y = y)) +
  geom_raster(aes(fill = iucn_pres), alpha = .5) +
  geom_raster(aes(fill = am_prob), alpha = .5) + 
