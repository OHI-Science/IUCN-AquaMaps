...
>>>>>>> 82d9692a1d3ff2bc04d3419a36d72a1dc650268c
...
for (i in 1:length(spp_quads)) { # i = 1
  ...
  ggsave(filename = file.path(dir_fig, sprintf('rangemap_%s_%s.png', spp_quad, str_replace(tolower(spp), ' ', '_'))),
         plot = spp_plot, 
...
Plotting species from q1 (well-matched)
...
hcaf <- read_csv(file.path(dir_anx,'raw/aquamaps_2014/tables/hcaf.csv')) #bring this in just to get loiczid
...
all_am_map = subs(loiczid_raster, all_am[ , c('loiczid', 'count')], 
                     by = 'loiczid', 
                     which = 'count', 
                     subsWithNA = TRUE)

levelplot(all_am_map,par.settings=myTheme,main='Entire Aquamaps Dataset',colorkey=list(at=seq(0, 1063, length=15)))
```
