...
spp_list <- spp_list %>%
  left_join(read.csv('sciname_lookups/spp_groupnames.csv', stringsAsFactors = FALSE),
            by = 'spp_group')
...
spp_risk_quadrants <- spp_risk_q1 %>%
  full_join(spp_risk_q2, by = c('popn_category', 'n_spp')) %>%
  full_join(spp_risk_q3, by = c('popn_category', 'n_spp')) %>%
  full_join(spp_risk_q4, by = c('popn_category', 'n_spp')) %>%
  gather(quad, n_quad, n_spp_q1, n_spp_q2, n_spp_q3, n_spp_q4) %>% 
  mutate(quad = str_replace(quad, 'n_spp_', ''),
         pct_quad = n_quad/n_spp,
         n_quad   = ifelse(is.na(n_quad),   0, n_quad),
...
spp_risk_quadrants <- spp_risk_quadrants %>%
  ...
  left_join(spp_list %>%
              ...
              unique() %>%
              mutate(cat_rank = ifelse(popn_category == 'DD', -1, cat_rank)),
            by = 'popn_category')

### Plot the bar chart
barchart_spp_risk_quads <- ggplot(spp_risk_quadrants %>% 
  ...
  labs(x = 'Risk category', 
       y = 'Relative number of species per quadrant', 
       title = 'Risk category breakdown by quadrant',
       fill = 'Match quadrant')
...
``` {r child = 'R/data_explore_pt3.Rmd', eval = FALSE}
```
...
``` {r breakdown by taxa, echo = FALSE, message = FALSE, warning = FALSE}

sciname_lookup <- read.csv('sciname_lookups/spp_iucn_all.csv', stringsAsFactors = FALSE)

iucn_maps_class <- spp_iucn_maps %>% 
  left_join(sciname_lookup %>% 
              select(sciname, class) %>%
              unique(), 
            by = 'sciname')
...
taxa_am_all_assessed <- spp_am %>%
  filter(!class %in% c('Not assigned', 'null') & !is.na(class)) %>%
  filter(sciname %in% spp_iucn_marine$sciname) %>%
  mutate(class = str_to_title(class),
         n_am_tot = n()) %>%        ### length of whole AM maps list
  group_by(class) %>%
...
knitr::kable(taxa_df %>%
               select(class, AquaMaps = n_am_class_all, IUCN = n_iucn_class_all, Both = n_class_both))
```

``` {r plot bar charts by taxa, echo = FALSE, message = FALSE, warning = FALSE}
...
taxa_df_long2 <- taxa_df %>%
  select(class, AquaMaps = n_am_class_solo, IUCN = n_iucn_class_solo, Both = n_class_both) %>%
  gather(dataset, n_spp, AquaMaps:Both) %>%
  group_by(class) %>%
...
bar_taxa2 <- ggplot(taxa_df_long2, aes(x = class, y = percent, fill = dataset)) +
  ...
  geom_text(aes(label = sprintf('n = %s', n_total), y = .05), hjust = 0, size = 4, angle = 90, hjust = 1) +
  theme(axis.text.x = element_text(angle = 75, hjust = 1)) + 
  labs(x = 'Class', y = 'Percent of mapped species',
       title = 'Distribution of species maps by taxonomic group (class)')
print(bar_taxa2)
ggsave(filename = file.path(dir_fig, 'bar_by_taxa_all.png'), height = 4, width = 8)

taxa_df_long3 <- taxa_df %>%
  select(class, AquaMaps = n_am_class_solo_ass, IUCN = n_iucn_class_solo, Both = n_class_both) %>%
  gather(dataset, n_spp, AquaMaps:Both) %>%
  group_by(class) %>%
  mutate(n_total = sum(n_spp, na.rm = TRUE),
         percent = n_spp/n_total * 100)
bar_taxa3 <- ggplot(taxa_df_long3, aes(x = class, y = percent, fill = dataset)) +
  ...
  geom_text(aes(label = sprintf('n = %s', n_total), y = .05), hjust = 0, size = 4, angle = 90, hjust = 1) +
  theme(axis.text.x = element_text(angle = 75, hjust = 1)) + 
  labs(x = 'Class', y = 'Percent of mapped and assessed species',
       title = 'Distribution of species maps by taxonomic group (class)')
print(bar_taxa3)
ggsave(filename = file.path(dir_fig, 'bar_by_taxa_assessed.png'), height = 4, width = 8)
...
``` {r, child = 'R/data_explore_global_maps.Rmd', eval = FALSE}
```
