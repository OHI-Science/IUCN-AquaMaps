...
reg <- ggplot(spp_list, aes(sm_perc,total_area,color=sm_range)) + geom_point() + geom_abline() + labs(y='Total Area', x='Percent overlap smaller range in larger', title='Comparing differences in area across species groups', fill='IUCN Category')

print(reg)

model = lm(spp_list$total_area~spp_list$sm_perc)
summary(model)
