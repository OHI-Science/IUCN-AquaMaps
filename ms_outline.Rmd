...
| IUCN      | `r nrow(spp_iucn_maps)` | `r nrow(spp_iucn_maps %>% filter(sciname %in% spp_iucn_marine$sciname))`  |
| Both      | `r nrow(spp_am %>% filter(sciname %in% spp_iucn_maps$sciname))` | `r nrow(spp_map_pairs)`           |
...
Focus on IUCN red-list evaluated species (incl or not incl DD)? or all species?

## Taxonomic distribution between data sets

Examine the overlapping and non-overlapping species by taxonomic group (order? family?) - are there differences in taxonomic group coverage between the two data sets?

IUCN - translate the taxonomic group into families - is there a way to automate this? http://www.itis.gov/downloads/?
AM - access taxa group information directly from AM species list - order? family?

``` {r breakdown by taxa, echo = FALSE}

sciname_lookup <- read.csv('sciname_lookups/scinames.csv', stringsAsFactors = FALSE)

iucn_maps_class <- spp_iucn_maps %>% 
  ### add sciname list, then jump through hoops to delete dupes
  ### NOTE: this also ditches ones with no sciname match...
  separate(sciname, 'genus', sep = ' ', remove = FALSE, extra = 'drop') %>%
  mutate(genus = tolower(genus)) %>%
  left_join(sciname_lookup, by = 'genus') %>% 
  filter(!(str_detect(tolower(spp_group), 'grass')    & !str_detect(kingdom, 'plant'))) %>%
  filter(!(str_detect(tolower(spp_group), 'mangrove') & !str_detect(kingdom, 'plant'))) %>%
  filter(!(str_detect(tolower(spp_group), 'coral')    & !str_detect(kingdom, 'animal'))) %>%
  filter(!(str_detect(tolower(spp_group), 'reptile')  & !str_detect(kingdom, 'animal'))) %>%
  filter(!(str_detect(tolower(spp_group), 'seabream') & !str_detect(kingdom, 'animal'))) %>%
  filter(!(str_detect(tolower(spp_group), 'wrass')    & !str_detect(kingdom, 'animal'))) %>%
  filter(!(str_detect(tolower(spp_group), 'mamm')     & !str_detect(kingdom, 'animal'))) %>%
  filter(!is.na(phylum)) %>%
...
knitr::kable(taxa_df %>%
               select(class, am_perc = p_am_class_all, iucn_perc = p_iucn_class_all, both_perc = p_class_both))
```
