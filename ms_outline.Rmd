...
<!--define what "assessed" means? include DD? keeping those in for now--> but only **4463** <!--11975 maps in those shapefiles, but fewer than 4500 correspond to assessed marine species... and only 2500 of those overall correpond to AquaMaps species--> of these have associated spatial information. AquaMaps has developed range maps for over 22,800 species. Only 2,100 species have been mapped in both datasets. This gap in overlapping species between these two datasets further highlights the need for understanding each of these datasets. Both IUCN and AquaMaps have gaps in marine species information and a significant portion of available data may be overlooked if limited to just one of these datasets.
...
___number of species in each data set, and the number that overlap___ 

* this highlights that each provides valuable and unduplicated spatial range information.

#### Summarize this section?

Researchers have two large and useful data sets at their disposal, each with its own utility.  Being able to use both simultaneously makes many more species available, but here there be challenges.
...
For each of the ___2200___ species whose range is described by both data sets, we calculated the distribution match and the area ratio, and plotted the ___2200___ species map pairs on a scatter plot of distribution alignment vs area alignment (Figure 1). 

![](figures/AreaRatiovsPercOverlap.png)

___Figure 1: Distribution alignment vs area alignment for ___2200___ species mapped in both Aquamaps and IUCN species distribution maps.___
...
| :----------------------: | :-----------------------: | :---------------: |
| q1: Well-aligned         | Distribution match > XX % | Area match > YY % |
| q2: Distribution-aligned | Distribution match < XX % | Area match > YY % |
| q3: Area-aligned         | Distribution match > XX % | Area match < YY % |
| q4: Poorly-aligned       | Distribution match < XX % | Area match < YY % |

Clearly the upper right quadrant (quadrant 1) is the place to be.  Species in this quadrant generally agree in spatial distribution between the two distribution maps, and the area of described ranges will be similar; expert-drawn range map and bioclimatic envelope model produce similar predictions of species distribution. (n = XXX; YYY %)

The other quadrants each suggest a different story.  Species map pairs that fall within the upper left quadrant (quadrant 2: well-matched in distribution but poorly-matched in area) indicate species in which the range of the smaller map falls generally within the larger map, but the larger map may include more generous buffers or may include areas unrepresented in the smaller map. (n = XXX; YYY %)

The lower right quadrant (quadrant 3: well-matched in area but poorly matched in distribution) includes species for which the two maps generally agree in range area, but disagree in where those ranges occur.  General disagreement in distribution seems to indicate a greater issue than general disagreement in area extent, so we categorized map pairs in this quadrant as being more problematic than map pairs in either of the upper quadrants. (n = XXX; YYY %)

The lower left quadrant (quadrant 4: poorly matched in both dimensions) indicates species for which the map pairs fail to agree in both area and distribution. (n = XXX; YYY %)
...
Figure 2 breaks the ___2200___ species map pairs into taxonomic groups as classified by the published IUCN range maps.  We were not surprised to find that map pairs for marine mammals, tunas, and billfishes were well-matched (quadrants 1 and 2); many of these species are global-ranging pelagic organisms, with some ranges encompassing nearly the entire ocean.  Mangroves are also not a surprise, being immobile and easily mapped by remote sensing methods.
