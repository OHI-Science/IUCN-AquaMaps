...
write_csv(iucn_coral_fixed, file.path(dir_git, 'coralmaps', 'iucn_coral_fixed.csv'))
...
ggsave(plot = clipped_quads,
       filename = file.path(dir_fig, 'fig3b_scatter_quadmap_corals.png'), device = 'png',
...
iucn_coral_fixed <- read_csv(file.path(dir_git, 'coralmaps', 'iucn_coral_fixed.csv'))

### identify quads based on original breaks from data_explore.Rmd

area_align_mean <- 54.53334
dist_align_mean <- 62.98212
...
iucn_coral_quads <- iucn_coral_fixed %>%
  select(iucn_sid, sciname, d_perc, a_ratio, method) %>% ### ditch am_sid, b/c duplicates
  distinct() %>%
  mutate(quad = 'q1',
...
iucn_coral_quads <- iucn_coral_quads %>%
  left_join(quad_names, by = 'quad')

write_csv(iucn_coral_quads, file.path(dir_git, 'coralmaps', 'iucn_coral_quads.csv'))

break_nums <- seq(0, 100, 20)

iucn_coral_quad_sum <- read_csv(file.path(dir_git, 'coralmaps', 'iucn_coral_quads.csv')) %>%
  select(method, quad, quad_name, pct_quad) %>%
  distinct()
### Plot the bar chart
...
ggsave(plot = barchart_coral_quads,
       filename = file.path(dir_fig, 'fig3c_barchart_coral_quads.png'),
       width = 17.8, height = 4, units = 'cm', dpi = 600)

write_csv(iucn_coral_quad_sum, file.path(dir_git, 'coralmaps', 'iucn_coral_quad_sum.csv'))
