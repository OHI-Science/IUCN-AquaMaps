...
spp_coral_clip <- read_csv(file.path(dir_git, 'coralmaps', 'iucn_coral_areas.csv')) %>%
  select(iucn_sid, shallow_pct, totarea, deeparea) %>%
...
scatterplot1 <- ggplot(spp_coral_clip, aes(x = totalarea, y = deeparea)) +
...
spp_coral_clip2 <- spp_coral_clip %>%
  select(iucn_sid, shallow = shallow_pct) %>%
  arrange(desc(shallow))

spp_coral_clip2$l_to_h <- c(1:nrow(spp_coral_clip))

spp_coral_clip2 <- spp_coral_clip2 %>%
  mutate(deep = 100 - shallow) %>%
  gather(depth, pct_value, c(shallow, deep)) %>%
  mutate(depth = ifelse(depth == 'shallow', '0 - 200 m', '200 m +'))

break_nums <- seq(0, 100, 20)
coral_area_plot <- ggplot(spp_coral_clip2, aes(x = l_to_h, y = pct_value, fill = depth)) +
  ggtheme_plot +
  # geom_linerange(aes(ymin = kept_ratio, ymax = 100),
  #                position = position_dodge(width = 0.5)) +
  geom_area() +
  scale_fill_manual(values = c('0 - 200 m' = 'grey90', '200 m +' = 'grey10')) +
  geom_hline(yintercept = break_nums, color = 'white', size = .25) +
  scale_y_continuous(limits = c(0, 100), breaks = break_nums,
                     labels = paste0(break_nums, '%')) +
  # scale_x_discrete(labels = element_blank()) +
  # theme(axis.title.x = element_blank()) +
  labs(y = 'IUCN % of coral range',
       x = 'Count of coral species')
